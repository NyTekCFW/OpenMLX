...
/*   Updated: 2024/06/29 00:47:51 by lchiva           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/// @param export_file the exported video name
...
void	init_record(char *export_file, char *track_img, int rate)
{
	...
	if (lx)
	{
		...
		if (rec && rec->status == 0)
		{
			...
			rec->status = 1;
			rec->rate = rate;
			xstrcpy(rec->buffer, export_file);
			rec->fd = open(rec->buffer, O_RDWR | O_CREAT | O_TRUNC, 0777);
			if (rec->fd <= 0)
			{
				rec->status = 0;
